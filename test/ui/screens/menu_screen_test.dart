...
void main() {
  ...
  testWidgets("Verify if exist a medium mode button in the menu", (widgetTester) async {
    await widgetTester.pumpWidget(const MaterialApp(home: MenuScreen(),));
    await widgetTester.pump();

    final customButtomEasyMode = find.widgetWithText(CustomMenuButtonWidget, "MEDIO");

    expect(customButtomEasyMode, findsOneWidget);
  });

  testWidgets("Verify if exist a hard mode button in the menu", (widgetTester) async {
    await widgetTester.pumpWidget(const MaterialApp(home: MenuScreen(),));
    await widgetTester.pump();

    final customButtomEasyMode = find.widgetWithText(CustomMenuButtonWidget, "DIFICIL");
